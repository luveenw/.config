...
function collectUncommitted({ cwd, log: log33 = import_npmlog.default }) {
  log33.silly("collect-uncommitted", "git status --porcelain (async)");
...
function npmRunScript(script, { args, npmClient, pkg, reject = true }) {
  import_npmlog25.default.silly("npmRunScript", script, args, pkg.name);
...
function npmRunScriptStreaming(script, { args, npmClient, pkg, prefix, reject = true }) {
  import_npmlog25.default.silly("npmRunScriptStreaming", [script, args, pkg.name]);
  const argv = ["run", script, ...args];
  const opts = makeOpts(pkg, reject);
  return childProcess7.spawnStreaming(npmClient, argv, opts, prefix && pkg.name);
}
...
var init_profiler = __esm({
  "libs/core/src/lib/profiler.ts"() {
    ...
    Profiler = class {
      ...
      outputPath;
      threads;
      constructor({ concurrency, log: log33 = import_npmlog28.default, outputDirectory }) {
        this.events = [];
        this.logger = log33;
