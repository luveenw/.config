...
function getCurrentSHA(opts) {
  import_npmlog35.default.silly("getCurrentSHA", "");
  const sha = childProcess9.execSync("git", ["rev-parse", "HEAD"], opts);
  import_npmlog35.default.verbose("getCurrentSHA", sha);
  return sha;
}
...
async function getCurrentTags(execOpts, matchingPattern) {
  import_npmlog36.default.silly("getCurrentTags", "matching %j", matchingPattern);
  const opts = Object.assign({}, execOpts, {
    // don't reject due to non-zero exit code when there are no results
    reject: false
  });
  const result = await childProcess10.exec(
    "git",
...
var import_npm_package_arg7, import_npmlog36, childProcess10;
...
async function getProjectsWithTaggedPackages(projectNodes, rootPath, execOpts) {
  ...
  const manifests = stdout.split("\n").filter((fp) => import_path20.default.basename(fp) === "package.json");
  const locations = new Set(manifests.map((fp) => import_path20.default.join(rootPath, import_path20.default.dirname(fp))));
  return projectNodes.filter((node) => locations.has(getPackage(node).location));
}
var import_npmlog38, import_path20, childProcess11;
...
async function getProjectsWithUnpublishedPackages(projectNodes, opts) {
  import_npmlog39.default.silly("getProjectsWithUnpublishedPackages", "");
  const mapper = (node) => {
    ...
    return import_pacote.default.packument(pkg.name, opts).then(
      ...
      () => {
        import_npmlog39.default.warn("", "Unable to determine published version, assuming %j unpublished.", pkg.name);
        return node;
      }
    );
  };
...
var import_npmlog39, import_p_map4, import_pacote;
...
function gitCheckout(stagedFiles, gitOpts, execOpts) {
  ...
  import_npmlog40.default.silly("gitCheckout", files);
  return childProcess12.exec("git", ["checkout", "--"].concat(files), execOpts);
}
...
function resolvePrettier() {
  if (!resolvedPrettier) {
    try {
      ...
      const prettierPath = import_path21.default.join(import_devkit6.workspaceRoot, "node_modules", "prettier");
...
var import_devkit6, import_fs4, import_npmlog42, import_path21, import_slash2, childProcess14, resolvedPrettier;
...
var import_devkit7, import_load_json_file3, import_npmlog49, import_path22;
