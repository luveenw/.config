...
var import_devkit4, import_clone_deep, import_dedent3, import_lodash4, import_npmlog12, import_client, import_os2, DEFAULT_CONCURRENCY, Command;
...
function getCycles(dependencies) {
  ...
  function dfs(next, path20) {
    ...
    for (const dep of dependencies[next] || []) {
      if (path20.includes(dep.target)) {
        const cycle = path20.slice(path20.indexOf(dep.target));
        cycles.push(cycle);
      } else if (!visited.has(dep.target)) {
        dfs(dep.target, path20);
      }
    }
    path20.pop();
  }
...
function getNpmExecOpts(pkg, registry) {
  ...
  import_npmlog20.default.silly("getNpmExecOpts", pkg.location, registry);
  return {
    cwd: pkg.location,
    env,
    pkg
...
function npmInstallDependencies(pkg, dependencies, config) {
  import_npmlog21.default.silly("npmInstallDependencies", pkg.name, dependencies);
  ...
  import_npmlog21.default.silly("npmInstallDependencies", "backup", pkg.manifestLocation);
  return import_fs_extra4.default.copy(pkg.manifestLocation, packageJsonBkp).then(() => {
    const cleanup = () => {
      import_npmlog21.default.silly("npmInstallDependencies", "cleanup", pkg.manifestLocation);
  ...
  return queue.add(
    async () => runScript({
      ...
      (err) => {
        ...
        import_npmlog23.default.error("lifecycle", "%j errored in %j, exiting %d", stage, pkg.name, exitCode);
...
var import_fs_extra5, import_libnpmpublish, import_npm_package_arg5, import_npmlog24, import_path13, import_pify2, readJSON, readJSONAsync;
...
var import_npm_packlist, import_npmlog27, import_path16, import_tar2;
...
var import_fs_extra7, import_npmlog28, import_upath;
...
var import_npmlog30, import_rimraf;
...
var import_node_fetch, import_npmlog32, import_path17, GitLabClient;
...
function getCurrentBranch(opts) {
  import_npmlog35.default.silly("getCurrentBranch");
  const branch = childProcess9.execSync("git", ["rev-parse", "--abbrev-ref", "HEAD"], opts);
  import_npmlog35.default.verbose("currentBranch", branch);
  return branch;
}
...
function maybeFormatFile(filePath) {
  ...
  if (resolvedPrettier.getFileInfo.sync(fullFilePath, { ignorePath }).ignored) {
    import_npmlog36.default.silly("version", `Skipped applying prettier to ignored file: ${filePath}`);
    return;
  }
  try {
    ...
    import_npmlog36.default.silly("version", `Successfully applied prettier to updated file: ${filePath}`);
  } catch {
    import_npmlog36.default.silly("version", `Failed to apply prettier to updated file: ${filePath}`);
  }
}
function gitAdd(changedFiles, gitOpts, execOpts) {
  ...
  import_npmlog36.default.silly("gitAdd", files);
  return childProcess10.exec("git", ["add", "--", ...files], execOpts);
}
var import_devkit5, import_fs4, import_npmlog36, import_path18, import_slash2, childProcess10, resolvedPrettier;
...
function gitCommit(message, { amend, commitHooks, signGitCommit, signoffGitCommit }, opts) {
  import_npmlog37.default.silly("gitCommit", message);
  const args = ["commit"];
  ...
  import_npmlog37.default.verbose("git", args);
  return childProcess11.exec("git", args, opts);
}
var import_npmlog37, import_os3, childProcess11;
...
function gitPush(remote, branch, opts) {
  ...
  return childProcess12.exec("git", ["push", "--follow-tags", "--no-verify", "--atomic", remote, branch], opts).catch((error) => {
    if (/atomic/.test(error.stderr) || process.env.GIT_REDIRECT_STDERR === "2>&1" && /atomic/.test(error.stdout)) {
      ...
      return childProcess12.exec("git", ["push", "--follow-tags", "--no-verify", remote, branch], opts);
    }
    throw error;
  });
}
var import_npmlog38, childProcess12;
...
function gitTag(tag, { forceGitTag, signGitTag }, opts, command = "git tag %s -m %s") {
  import_npmlog39.default.silly("gitTag", tag, command);
  ...
  import_npmlog39.default.verbose(cmd, interpolatedArgs);
  return childProcess13.exec(cmd, interpolatedArgs, opts);
}
...
function isAnythingCommitted(opts) {
  import_npmlog40.default.silly("isAnythingCommitted", "");
  const anyCommits = childProcess14.execSync("git", ["rev-list", "--count", "--all", "--max-count=1"], opts);
  import_npmlog40.default.verbose("isAnythingCommitted", anyCommits);
...
function updateRemote(opts) {
  childProcess15.execSync("git", ["remote", "update"], opts);
}
function countLeftRight(symmetricDifference, opts) {
  const stdout = childProcess15.execSync(
    "git",
...
function remoteBranchExists(gitRemote, branch, opts) {
  import_npmlog42.default.silly("remoteBranchExists", "");
  const remoteBranch = `${gitRemote}/${branch}`;
  try {
    childProcess16.execSync("git", ["show-ref", "--verify", `refs/remotes/${remoteBranch}`], opts);
    return true;
  } catch (e) {
    return false;
  }
...
async function updateLockfileVersion(pkg) {
  ...
  const obj = await (0, import_load_json_file3.default)(lockfilePath).catch(() => {
    import_npmlog43.default.verbose("version", `${pkg.name} has no lockfile. Skipping lockfile update.`);
  });
  if (!obj) {
    return;
  }
...
var import_devkit6, import_load_json_file3, import_npmlog43, import_path19;
      ...
      getVersionsForUpdates() {
        ...
        if (repoVersion) {
          ...
          const nextVersion = applyBuildMetadata(
            import_semver8.default.inc(this.project.version, increment, resolvePrereleaseId(prereleaseId)),
            this.options.buildMetadata
          );
...
function getCurrentSHA(opts) {
  import_npmlog44.default.silly("getCurrentSHA", "");
  const sha = childProcess17.execSync("git", ["rev-parse", "HEAD"], opts);
  import_npmlog44.default.verbose("getCurrentSHA", sha);
  return sha;
}
...
async function getCurrentTags(execOpts, matchingPattern) {
  import_npmlog45.default.silly("getCurrentTags", "matching %j", matchingPattern);
  const opts = Object.assign({}, execOpts, {
    // don't reject due to non-zero exit code when there are no results
    reject: false
  });
  const result = await childProcess18.exec(
    "git",
...
var import_npm_package_arg7, import_npmlog45, childProcess18;
...
async function getProjectsWithTaggedPackages(projectNodes, rootPath, execOpts) {
  import_npmlog47.default.silly("getTaggedPackages", "");
  ...
  const manifests = stdout.split("\n").filter((fp) => import_path22.default.basename(fp) === "package.json");
  const locations = new Set(manifests.map((fp) => import_path22.default.join(rootPath, import_path22.default.dirname(fp))));
  return projectNodes.filter((node) => locations.has(getPackage(node).location));
}
var import_npmlog47, import_path22, childProcess19;
...
async function getProjectsWithUnpublishedPackages(projectNodes, opts) {
  import_npmlog48.default.silly("getProjectsWithUnpublishedPackages", "");
  const mapper = (node) => {
    const pkg = getPackage(node);
    return import_pacote.default.packument(pkg.name, opts).then(
      ...
      () => {
        import_npmlog48.default.warn("", "Unable to determine published version, assuming %j unpublished.", pkg.name);
        return node;
      }
    );
  };
...
var import_npmlog48, import_p_map4, import_pacote;
...
function gitCheckout(stagedFiles, gitOpts, execOpts) {
  ...
  import_npmlog49.default.silly("gitCheckout", files);
  return childProcess20.exec("git", ["checkout", "--"].concat(files), execOpts);
}
  ...
  "libs/commands/publish/src/index.ts"(exports2, module2) {
    ...
    var PublishCommand = class extends Command {
      ...
      async resetChanges() {
        ...
        const dirtyManifests = [this.project.manifest].concat(this.packagesToPublish).map((pkg) => import_path23.default.relative(_workspaceRoot, pkg.manifestLocation));
      ...
      async copyAssets(pkg) {
        if ((0, import_path23.normalize)(pkg.location) === (0, import_path23.normalize)(pkg.contents)) {
          return;
        }
        const _workspaceRoot = process.env["NX_WORKSPACE_ROOT_PATH"] || import_devkit8.workspaceRoot;
        ...
        for (const asset of assets) {
          if (typeof asset === "string") {
            ...
            for (const file of files) {
              filesToCopy.push({
                from: (0, import_path23.join)(pkg.location, file),
                to: (0, import_path23.join)(pkg.contents, file)
              });
            }
            ...
            for (const file of files) {
              filesToCopy.push({
                from: (0, import_path23.join)(pkg.location, file),
                to: (0, import_path23.join)(pkg.contents, asset.to, (0, import_path23.basename)(file))
              });
            }
          } else {
            throw new ValidationError(
        ...
        for (const file of filesToCopy) {
          if ((0, import_path23.normalize)(file.from) === (0, import_path23.normalize)(file.to)) {
            this.logger.warn(
              "EPUBLISHASSET",
